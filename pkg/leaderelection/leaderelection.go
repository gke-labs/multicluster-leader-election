...
type LeaseInfo struct {
	...
	// LastRenewTime is when the lease was last renewed (may be nil if no lease exists).
...
func (le *LeaderElector) AcquireOrRenew(ctx context.Context, lease *v1alpha1.MultiClusterLease, identity string) (*LeaseInfo, error) {
	...
	if err != nil {
		...
		if createErr == nil {
			...
			return &LeaseInfo{
				...
				LastRenewTime:  &newLease.Data.LastRenewTime,
			}, nil
		}
	...
	log.Info("successfully read lease", "holder", leaseObject.Data.HolderIdentity, "renewTime", leaseObject.Data.LastRenewTime)
	leaseDuration := time.Duration(leaseObject.Data.LeaseDurationSeconds) * time.Second
	leaseExpired := time.Since(leaseObject.Data.LastRenewTime) > leaseDuration
	...
	if leaseObject.Data.HolderIdentity == identity || leaseExpired {
		...
		leaseObject.Data.LastRenewTime = time.Now()
		...
		if updateErr != nil {
			...
			return &LeaseInfo{
				...
				LastRenewTime:  &leaseObject.Data.LastRenewTime,
			}, nil
		}
