...
func TestReconcile_LeaseHeldByOther(t *testing.T) {
	...
	// Pre-populate the storage backend
	mockStorage, ok := reconciler.Storage.(*storage.InMemoryStorage)
	require.True(t, ok, "reconciler storage is not InMemoryStorage")
	err = mockStorage.CreateLease(&storage.LeaseObject{
...
func TestReconcile_BackendError(t *testing.T) {
	...
	// Inject an error into the mock storage
	mockStorage, ok := reconciler.Storage.(*storage.InMemoryStorage)
	require.True(t, ok, "reconciler storage is not InMemoryStorage")
...
func TestReconcile_ObjectDeletion(t *testing.T) {
	...
	lease := &v1alpha1.MultiClusterLease{
		ObjectMeta: metav1.ObjectMeta{
			Name:       leaseName,
			Namespace:  nsName,
			Finalizers: []string{"multiclusterlease.multicluster.core.cnrm.cloud.google.com/finalizer"},
		},
		Spec: v1alpha1.MultiClusterLeaseSpec{
			HolderIdentity:       &identity,
			LeaseDurationSeconds: &duration,
